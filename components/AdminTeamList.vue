<template>
    <table v-if="props.data !== null && props.data.length > 0" class="table">
        ...
        <tbody>
            <tr v-for="(team, index) in props.data" :key="index" class="hover">
                <td>{{ team.name }}</td>
                <td class="flex flex-row space-x-1 justify-end">
                    <button @click="$emit('edit', team)" class="group btn btn-square btn-ghost hover:bg-cyan-600">
                    ...
                    <button @click="$emit('delete', team)" class="group btn btn-square btn-ghost hover:bg-red-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-6 h-6 group-hover:stroke-white">
                            <path stroke-linecap="round" stroke-linejoin="round"
